...
3. Da li je ovo dobra procena mortaliteta?
...
## Standardna greška aritmetičke sredine i proporcije {.smaller}

Standardna greška [aritmetičke sredine]{.yellow}:

$$ SE_{\bar x} = \frac{sd}{\sqrt{n}} $$

gde je: $sd$, standardna devijacija statistike u uzorku (populaciona nije dostupna); $n$ broj opservacija.

Standardna greška [proporcije]{.yellow}:

$$ SE_{p} = \sqrt{\frac{p(1-p)}{n}} $$
gde je: $p$, proporcija događaja u uzorku; $n$ broj opservacija.
...
99% interval je [sigurniji]{.yellow} (ima manji
...
tribble(
  ...
  rowwise() |>
  mutate(
    `Stopa smrtnosti (%)` = round (Mortalitet / Ukupno * 100 , 1),
    `95% IP (%)` = paste(round(binom.test(x = Mortalitet, n = Ukupno)$conf.int[1] * 100, 2), " - ",
                         round(binom.test(x = Mortalitet, n = Ukupno)$conf.int[2] * 100, 2))) |>
      tt() |> style_tt(fontsize = 0.8) %>% style_tt(i = 0, fontsize = 0.75)
```
