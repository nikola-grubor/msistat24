...
[Kategorizovanje]{.yellow} kontinuiranih podataka dovodi do [gubitka informacija]{.yellow} i preciznosti

- "Cut-point" ne postoji u prirodi
- Čak i da postoji, optimalno odlučivanje se bazira na rizicima

![](slike/cutpoints.png){fig-align="center"}
...
![](slike/pvc.png){fig-align="center"}
...
Da li su VES [nezavisan faktor rizika]{.yellow} naprasne smrti?
...
::: {.column}
![](slike/cardriskmodel.jpg){width=90%}
:::

::::

## Prikaz slučaja: Antiaritmici (4) {.smaller}
...
::: {.column}
![](slike/efmodel.jpg){width=90%}
:::

::::

## CAST: Cardiac Arrhythmia Suppression Trial {.smaller}
...
Pogodno za analizu samo [kompletnih opservacija]{.yellow}.
...
Pogodno za analizu [regresionom/stohastičnom imputacijom]{.yellow} ako je model tačno specificiran.
...
Rešenje je obično [prikupljanje podataka o mehanizmu]{.yellow} direktno ili [analize senzitivnosti]{.yellow} za različite kontekste.
...
tribble(
  ...
  tt(notes = "*Samo ako je model tačno specificiran.") %>% 
  style_tt(i = 0:5, fontsize = 0.7) %>% 
  group_tt(j = list(
    "Nepristasno" = 2:4
  ))
```
