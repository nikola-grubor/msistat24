...
- **Discontinuous** (discrete) measurements (data) are [qualitative]{.yellow} or can be expressed with [integers]{.yellow}.
...
tt(scales,
   caption = "Allowed operations based on the scale") %>% 
...
::: {.callout-warning}
Ordinal and discontinuous data can look similar. The difference between them is that the numbers in ordinal data are only labels, not real values. Continuous and discontinuous data may look similar if rounded (50kg, 60kg, 75kg), but we always have the option to display them more precisely.
:::
